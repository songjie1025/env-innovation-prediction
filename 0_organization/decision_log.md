...
### 2026-07-07: Add manufacturing_share to main model predictor set

Decision:
Add `manufacturing_share` (WDI `NV.IND.MANF.ZS`, manufacturing value added as percent of GDP) to the main model predictor set, completing its pipeline integration that was already partially in place (panel data, variable map, evidence strength).

Reason:
The raw data was already downloaded and the v2 model panels already included `manufacturing_share_lag1_3_mean` (246 countries, 35 years 1990–2024, 5.7% missing in the main panel). The variable represents industrial structure as a potential driver of innovation patterns: manufacturing-intensive economies may differ systematically in their environmental patent profiles from service-based economies. Literature support draws on the "industrial commons" thesis (Cohen & Zysman 1987, *Manufacturing Matters*; Pisano & Shih 2009/2012) arguing that manufacturing capability constitutes a structural foundation for innovation capacity. The variable is already described in `data_dictionary.md` and `variable_framework.md`.

Implementation: (1) Added a literature-CSV row linking `NV.IND.MANF.ZS` to the Cohen-Zysman / Pisano-Shih references. (2) Registered the raw file in `raw_download_manifest.csv` and regenerated `raw_predictor_audit.csv` (206 entities, 35 years, reliability confirmed). (3) Re-ran statistical screening (VIF = 1.06, near-zero-variance: none) and the full linear + tree modeling pipeline. Results: linear Lasso keeps `manufacturing_share` coefficient at zero; tree impurity importance = 0.076 (modest, consistent with the earlier exploratory finding that manufacturing carries a nonlinear signal in RF but not in XGBoost or linear models). Spearman correlation with target = 0.325 (train+val). The addition does not change the headline conclusion (no model beats persistence baseline 0.312).

Alternatives considered:
Omitting manufacturing_share was rejected because it was already in the v2 panels and variable map but missing from the literature CSV, manifest, and audit — a documentation gap, not a substantive exclusion. Keeping it only as an exploratory variable was rejected because the Cohen-Zysman / Pisano-Shih references provide sufficient theoretical grounding for inclusion as a structural control. Removing it after seeing the zero Lasso coefficient was rejected as post-hoc variable deletion.

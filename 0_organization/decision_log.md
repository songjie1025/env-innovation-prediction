...
Decision:
Add an explicit Phase-3 statistical screening step — near-zero variance, predictor-predictor correlation, and variance inflation factor (VIF) — as a data-side diagnostic. The new computations (VIF, near-zero variance) live in `2_data/scripts/statistical_screening.py`; the correlation matrix, top correlated pairs, and predictor-target correlations are reused from the existing leakage-safe train+validation diagnostics produced by `run_modeling.py` rather than recomputed. A visualization-only notebook `2_data/notebooks/statistical_screening.ipynb` calls the script and displays the results. The predictor set is unchanged at 18 (main 9, SubA 3, SubB 5, SubC 1).

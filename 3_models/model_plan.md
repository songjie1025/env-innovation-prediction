...
The persistence-adjusted target extension additionally includes a block-safe delta-to-level correction protocol. The first layer keeps the family comparison unchanged by selecting each model family using validation MAE on `delta_lag1`; the final model-selection notebook then ranks candidate delta specifications using validation corrected-level MAE only. Validation corrected-level forecasts anchor each country at its latest training-period target and recursively add predicted validation deltas. Once a specification is fixed by validation, the script refits it on train+validation and reports a held-out test correction audit using the latest pre-test country target as the anchor. Rows whose pre-forecast anchor is not consecutive with the first forecasted year are flagged and excluded from corrected-level MAE, so one-year delta predictions are not used to bridge multi-year target gaps. The diagnostic is reported against the same-sample hold-constant country-history baseline on the level target scale and should not be described as test-set model selection.

...
The catalogs are reviewer-facing discovery artifacts. They should be read together with `0_organization/decision_log.md`: the main target is now fixed as `PT_INV.DEV.ENV_PAT._Z` / `env_patent_share_inventions`, while predictor selection remains a classification task across main-model, robustness, exploratory, and data-limited roles.
...
3. The predictor catalog is provisional because the literature review and the manual CSV screening pool are still being reconciled.
4. Predictor concepts from `1_literature_review/Managerial AI- literature review - List 1.csv` should remain in consideration until assigned to main-model, robustness, exploratory, or data-limited roles.
